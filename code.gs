...
// POST 요청 처리 (반 등록, 로그인, 기사 저장) - CORS 및 데이터 수신 오류 원천 차단
function doPost(e) {
  try {
    let data = {};
    ...
    if (action === 'registerTeacher') {
      ...
      for (let i = 1; i < rows.length; i++) {
        if (String(rows[i][0]) === className && String(rows[i][1]) === password) {
...
// 브라우저 간 통신 및 CORS 차단을 우회하기 위한 JSON 출력 함수
